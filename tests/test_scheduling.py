...
class ProfessionalScheduleTest(unittest.TestCase):
    def test_policy_normalizes_priority_attempts_and_exceptions(self) -> None:
        schedule = normalize_schedule(
            {
                "enabled": True,
                "entries": [{"weekday": 0, "time": "08:00"}],
                "exceptions": [{"date": "2026-08-17", "action": "skip"}],
                "priority": 9,
                "max_attempts": 0,
    ...
    def test_scheduler_prioritizes_due_workflows(self) -> None:
        ...
        scheduler = WorkflowScheduler(
            catalog, lambda _company, workflow: order.append(workflow) or {"ok": True}
        )
    ...
    def test_schedule_center_builds_queue_calendar_and_forecast(self) -> None:
        ...
        reports = [
            {
                "company": "sol",
                "workflow_ids": ["cadastro"],
                "started_at": "2026-08-13T08:00:00",
                "finished_at": "2026-08-13T08:10:00",
            }
        ]
