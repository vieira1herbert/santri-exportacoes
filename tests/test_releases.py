...
class ReleaseManagementTest(unittest.TestCase):
    def test_check_uses_selected_channel_and_reports_no_published_release(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                "santri_automation.services.release_manager.urllib.request.urlopen",
    ...
    def test_preferences_isolate_environment_and_channel(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            saved = manager.save_preferences(
                {
    ...
    def test_prepare_update_backs_up_and_verifies_executable(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest = json.dumps(
                {"version": "1.8.0", "executable": {"sha256": digest}}
            ).encode()
            manager = TestableReleaseManager(
                root,
                "1.7.0",
                root / "notes.md",
                downloads={"manifest": manifest, "executable": executable},
            )
            ...
            release = {
                ...
                "assets": [
                    {"name": "santri-exportacoes-release.json", "url": "manifest"},
                    {"name": "Santri Exportações.exe", "url": "executable"},
                ],
            }
    ...
    def test_prepared_upgrade_is_not_a_rollback_and_current_release_is_preserved(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            manager = TestableReleaseManager(
                ...
                "1.7.0",
                root / "notes.md",
                downloads={"manifest": manifest, "executable": next_executable},
            )
            release = {
                "latest_version": "1.8.0",
                "assets": [
            ...
            with (
                ...
                patch.object(sys, "executable", str(current_executable)),
            ):
    ...
    def test_prepare_update_rejects_hash_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            manifest = json.dumps(
                {"version": "1.8.0", "executable": {"sha256": "0" * 64}}
            ).encode()
            manager = TestableReleaseManager(
                root,
                "1.7.0",
                root / "notes.md",
                downloads={"manifest": manifest, "executable": b"changed"},
            )
            release = {
                "latest_version": "1.8.0",
                "assets": [
